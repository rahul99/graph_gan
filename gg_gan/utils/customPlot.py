...
def plot_generator_output(n_ex=5):
	# Compare generator output versus target data versus condition image
	random_samples = np.random.randint(0,global_variable.gen_inps_pair.shape[0],size=n_ex)
	condition_image = global_variable.cond_data[random_samples,:,:,:]
	ground_truth = global_variable.trgt_data[random_samples,:,:,:]
